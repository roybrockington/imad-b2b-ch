...
export default function EditCareerPage() {
  ...
  const careerId = parseInt(params.id as string);
  ...
  useEffect(() => {
    ...
    const fetchCareer = async () => {
      try {
        ...
        const data = await api.getCareer(careerId);
        setCareer(data);

        // Format start_date to YYYY-MM-DD for HTML date input
        const formatDateForInput = (dateString: string) => {
          const date = new Date(dateString);
          return date.toISOString().split('T')[0];
        };

        // Load German values by default
        setFormData({
          position: data.position_de || '',
          start_date: formatDateForInput(data.start_date),
          location: data.location,
          tasks: data.tasks_de || '',
          profile: data.profile_de || '',
          expectations: data.expectations_de || '',
          published: data.published,
        });
      } catch (error: any) {
        console.error('Failed to fetch career:', error);
        setError(error.message || 'Failed to load career listing');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Map form data to German fields
      const updateData = {
      ...
      await api.updateCareer(careerId, updateData);
      router.push(`/${locale}/admin/careers`);
    } catch (error: any) {
      console.error('Failed to update career:', error);
      setError(error.message || 'Failed to update career listing');
    } finally {
      setLoading(false);
    }
  };

  if (!currentUser || fetchLoading) {
    return (
  ...
  if (!career) {
    return (
      <div className="min-h-screen bg-white">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="flex items-center justify-center h-64">
            <p className="text-lg text-red-500">Career listing not found</p>
          </div>
        </main>
      </div>
    );
